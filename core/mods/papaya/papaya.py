...
class CaptchonkaOCRMod(CaptchonkaOCR):
  def checkCharacterSizes(self, width=0, height=0):
    return (width > 5 or width * height >= 80) and height > 3 and width < 28 and height < 28
  ...
  def cleanImage(self, processed):
    ...
    processed = self.blackAndWhite(processed, 170)
  ...
  def cleanDenoise(self, processed):
    ...
    clean = cv2.fastNlMeansDenoising(numpy.array(processed), None, 35, 4, 7)
  ...
  def erodeAndDilate(self, processed):
    ...
    processed = contr.enhance(1.1)

    sharp = ImageEnhance.Sharpness(processed)
    processed = sharp.enhance(1.9)

    clean = cv2.fastNlMeansDenoising(numpy.array(processed), None, 10, 3, 7)
    processed = Image.fromarray(numpy.uint8(clean))
